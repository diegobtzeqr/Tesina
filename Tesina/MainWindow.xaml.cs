...
namespace Tesina
{
	...
	public partial class MainWindow : Window
	{
		...
		private void KinectSkeletonFrameReady(object sender, SkeletonFrameReadyEventArgs e)
		{
			...
			var leftDistToHipY = Math.Abs(leftHand.Position.Y - leftHip.Position.Y);
			var rightDistToHipY = Math.Abs(rightHand.Position.Y - rightHip.Position.Y);

			if (leftDistToHipY > 0.8 || rightDistToHipY > 0.8)
			{
				if (nt47Finished)
				{
					nt47Finished = false;
			...
			else if (leftDistToHipY < 0.1 || rightDistToHipY < 0.1)
			{
				if (nt13finished)
				{
					nt13finished = false;
			...
			soundsLeftArm(Math.Abs(leftHand.Position.X - leftShoulder.Position.X), Math.Abs(leftHand.Position.Y - leftShoulder.Position.Y),
						  Math.Abs(leftHand.Position.Z - leftShoulder.Position.Z), leftDistToHipY);
			soundsRightArm(Math.Abs(rightHand.Position.X - rightShoulder.Position.X), Math.Abs(rightHand.Position.Y - rightShoulder.Position.Y),
						   Math.Abs(rightHand.Position.Z - rightShoulder.Position.Z), rightDistToHipY);
